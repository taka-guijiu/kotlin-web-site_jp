...
Kotlinの関数は[第一級](https://en.wikipedia.org/wiki/First-class_function)であり、これは、変数やデータ構造に格納できるほか、他の[高階関数](#higher-order-functions)への引数として渡したり、それらから戻り値として受け取ったりできることを意味します。
関数に対しては、関数以外の値に対して可能なあらゆる操作を行うことができます。

これを容易にするため、静的型付けプログラミング言語であるKotlinでは、関数を表現するために一連の[関数型](#function-types)を使用し、[ラムダ式](#lambda-expressions-and-anonymous-functions)などの専用の言語構文を提供しています。
...
高階関数の良い例としては、コレクションに対する[関数型プログラミングのイディオム「fold」](https://en.wikipedia.org/wiki/Fold_(higher-order_function))が挙げられます。
初期の累積値と結合関数を受け取り、現在の累積値をコレクションの各要素と順次結合し、その都度累積値を置き換えることで、戻り値を生成します：
...
初期の累積値と結合関数を受け取り、現在の累積値をコレクションの各要素と順次結合し、その都度累積値を置き換えることで、戻り値を生成します：
これは `for` ループ内で [呼び出され(invoked)](#invoking-a-function-type-instance)、その戻り値が `accumulator` に代入されます。

`fold` を呼び出すには、引数として [関数型のインスタンス](#instantiating-a-function-type) を渡す必要があり、高階関数の呼び出し箇所では、この目的でラムダ式 ([詳細は後述する](#lambda-expressions-and-anonymous-functions)) が広く用いられています：
